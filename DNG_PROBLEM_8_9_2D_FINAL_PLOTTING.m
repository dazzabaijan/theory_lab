...
marker = ['+']
% marker = ['+', '*', 'o', 'd']
J = linspace(1, 20, 20);%J points for the line graph plot
...
for loop = 1:length(mu)
        ...
        for i=1:Lx
            for j=1:Ly
                ...
                if j<k+1 %depth of wall
                    ...
                    rho(i,j) = 0;
                elseif j<(19+k)
                    rho_rhs(i,j) = (1 - rho(i,j))*exp((beta*(rho(i,j-1) + rho(i,j+1) + rho(left,j) + rho(right,j) + (1/4)*(rho(left,j-1) + rho(right,j-1) + rho(left,j+1) + rho(right,j+1)) + mu(loop)) - potential(j)));
                else
    ...
    plot(J, rho(1,1:20), 'LineStyle', '-', 'Marker', marker(loop), 'LineWidth', 1.5);
end
hold off;
